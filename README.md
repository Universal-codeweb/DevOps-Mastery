# DevOps & Full Stack Learning Journey

Documenting my daily progress through practical learning, experimentation, and hands-on implementation.

## Learning Log

### June 18, 2026 started

✅ **Day 1 – Foundations**

* Development environment setup
* Git & GitHub workflow
* Repository management
📄 [View detailed explanation](Day01-Setup-Orientation/18-06-2026.md)
📄 [View Source Code](index.html)
---

✅ **Day 2 – HTML Fundamentals**

* Webpage structure
* Semantic HTML
* Forms, tables, images, and navigation
📄 [View Source Code](form.html)
📄 [View Source Code](index.html)

---

✅ **Day 3 – CSS Fundamentals**

* Styling and layouts
* Selectors, classes, and IDs
* Box model, cards, navigation bars
* Hover effects and UI enhancement
📄 [View Source Code](form.css)
📄 [View Source Code](styles.css)

---
### June 19, 2026

✅ **Day 4 – CSS Layouts & Responsive Design**

* Flexbox fundamentals (`display: flex`)
* `flex-direction`, `justify-content`, `align-items`
* Perfect centering and spacing using `gap`
* `flex-wrap` for responsive layouts
* `flex-grow`, `flex-shrink`, `flex-basis`, `order`
* Responsive units (`%`, `vw`, `vh`, `em`, `rem`)
* Viewport meta tag
* Media Queries for mobile-friendly design
📄 [View Source Code](layout.css)

---

✅ **Day 5 – Git & Multi-Section Page**

* Git fundamentals and version control
* `git init`, `git status`, `git add`, `git commit`
* `git push`, `git pull`, GitHub integration
* Branch creation and switching
* Testing workflow using a dedicated `testing` branch
* Merging tested changes into `main`
* Building and styling a multi-section webpage

---
### June 22, 2026
✅ **Day 6 – JavaScript Variables, Types & Operators**

* Variable declarations using `let` and `const`
* JavaScript primitive and reference data types

  * Number
  * String
  * Boolean
  * Null
  * Symbol
  * Array
  * Object
* Arithmetic operators
* Comparison operators
* Object methods
* Understanding the `this` keyword
* Introduction to Regular Expressions (Regex)
* Case-insensitive matching using the `i` flag
* Practiced data manipulation using operators
📄 [View Source Code](expressions.js)
📄 [View Source Code](index.js)
📄 [View Source Code](script.js)

---
### June 23, 2026
✅ **Day 7 – JavaScript Functions**

* Function declaration
* Function expression
* Anonymous functions
* Named function expressions
* Arrow functions
* Immediately Invoked Function Expressions (IIFE)
* Callback functions
* Function parameters and return values
* Scope and variable accessibility
* Practical function implementation
📄 [View Source Code](fun.js)

---
### June 24, 2026
✅ **Day 8 – JavaScript Arrays & Objects**

### Arrays

* Creating arrays using literals and constructors
* Accessing array elements
* Array properties (`length`)
* Array destructuring
* Nested arrays
* Merging arrays using spread operator
* Swapping variables using destructuring

### Objects & Properties

* Creating objects
* Object properties
* Object methods
* Method shorthand
* Dot notation
* Bracket notation
* Adding object properties
* Updating object properties
* Deleting object properties
* Computed property names
* Object spread operator
* Object destructuring
📄 [View Source Code](arrays.js)
📄 [View Source Code](Objects.js)
---


✅ **Day 9 – JavaScript Loops & Conditional Statements**

* `for` loop
* `for...in`
* `for...of`
* `forEach()`
* `while` loop
* `do...while` loop
* Working with `Map`
* Reversing a number using loops
* `if`, `if...else`, `else if` ladder
* `switch` statement
* Iteration over arrays and objects
* Practical loop and conditional programming exercises
📄 [View Source Code](practice.js)

---

✅ **Day 10 – JavaScript DOM & Events**

* Introduction to the Document Object Model (DOM)
* Selecting elements using `getElementById()` and `querySelector()`
* Handling events using `onclick` and `addEventListener()`
* Preventing default form submission using `event.preventDefault()`
* Reading user input using `.value`
* Form validation
* Updating page content using `innerText`
* Dynamic style manipulation
* Password character counter
* Password strength indicator
* Creating HTML elements using `createElement()`
* Appending elements using `appendChild()`
* Understanding `click` and `input` events

📄 [View Source Code](signup.js)

---

✅ **Day 11 – Modern JavaScript (ES6)**

* ES6 Modules
* Named and default exports
* Importing modules
* Arrow functions
* Spread operator
* Rest operator
* Module-based code organization
* Practical implementation using multiple JavaScript files

📄 [View Source Code](main.js)
📄 [View Source Code](math.js)

---

⬜ **Day 12 – JavaScript Promises**

* Introduction to asynchronous programming
* Understanding the need for Promises
* Creating Promises
* Promise states

  * Pending
  * Fulfilled
  * Rejected
* Consuming Promises using `.then()`
* Handling errors using `.catch()`
* Executing code using `.finally()`
* Promise chaining
* Practical Promise implementation

📄 [View Source Code](promise.js)

✅ **Day 13 – Async/Await & Fetch API**

* Async functions
* Await keyword
* Sequential asynchronous execution
* Error handling using `try...catch`
* Fetch API
* HTTP GET requests
* Working with the Response object
* Parsing JSON using `response.json()`
* Fetching data from an API
* DOM manipulation using fetched data
* Practical implementation with a Pokémon Search application

📄 [View Source Code](asaw.js)  
📄 [View Source Code](poke.html)  
📄 [View Source Code](poke.js)

## ✅ Day 14 – TypeScript Introduction

* Introduction to TypeScript
* Installing and setting up TypeScript (`tsc`)
* Compiling `.ts` files to `.js`
* Running TypeScript with Node.js
* Static typing and type annotations
* Type inference
* Variables and data types (`string`, `number`, `boolean`)
* Arrays (`string[]`)
* Functions with `void` return type
* Practiced creating variables, arrays, functions, and console output
* Identified and fixed common TypeScript errors

📄 [View Source Code](intro.ts)

✅ **Day 15 – TypeScript Object Types & Interfaces**

* Object types and typed objects
* Creating reusable interfaces
* Using interfaces with objects
* Passing interfaces as function parameters
* Function return types (`void`, `number`)
* Implementing interfaces using classes
* Type safety and compile-time error checking
* Practical implementation with objects, interfaces, functions, and classes

📄 [View Source Code](practice15.ts)

---


## Progress Tracker

* [x] Day 1
* [x] Day 2
* [x] Day 3
* [x] Day 4
* [x] Day 5
* [x] Day 6
* [x] Day 7
* [x] Day 8
* [x] Day 9
* [x] Day 10
* [x] Day 11
* [x] Day 12
* [x] Day 13
* [x] Day 14
* [ ] Day 15